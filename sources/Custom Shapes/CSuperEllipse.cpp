#include "CSuperEllipse.h"

void CSuperEllipse::init()
{
    std::vector<sf::Vector2f> sin_4;
    constexpr float PREC = M_PI_2 / 15;
...
CSuperEllipse::CSuperEllipse(const sf::Rect<float>& rect, float radius, const sf::Color& color)
    : m_rect(rect), m_radius(radius)
{
    init();
...
size_t CSuperEllipse::getPointCount() const
{
    return m_points.size();
}

sf::Vector2f CSuperEllipse::getPoint(size_t index) const
{
